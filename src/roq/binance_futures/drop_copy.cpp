...
DropCopy::DropCopy(Handler &handler, core::io::Context &context,
    ...
    : handler_(handler), stream_id_(stream_id),
      ...
      connection_(*this, context, core::URI(Flags::ws_uri()),
                  create_query(listen_key), Flags::ws_ping_freq(),
                  Flags::decode_buffer_size(), Flags::encode_buffer_size(),
                  []() { return std::string(); }),
      ...
      profile_{
          ...
          .outbound_account_info =
              create_metrics(name_, "outbound_account_info"_sv),
          .outbound_account_position =
              create_metrics(name_, "outbound_account_position"_sv),
...
bool DropCopy::ready() const { return connection_.ready(); }

void DropCopy::operator()(const Event<Start> &) { connection_.start(); }

void DropCopy::operator()(const Event<Stop> &) { connection_.stop(); }
...
void DropCopy::operator()(const json::BalanceUpdate &balance_update,
...
void DropCopy::operator()(const json::ExecutionReport &execution_report,
                          const server::TraceInfo &trace_info) {
  profile_.execution_report([&]() {
    ...
    auto found = shared_.find_order(
        ...
        execution_report.client_order_id, order_lookup, trace_info,
