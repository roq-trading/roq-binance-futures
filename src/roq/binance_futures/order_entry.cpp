...
static const auto SUPPORTS = utils::Mask{
    SupportType::REFERENCE_DATA, SupportType::MARKET_STATUS,
    SupportType::CREATE_ORDER,   SupportType::CANCEL_ORDER,
    SupportType::ORDER_ACK,      SupportType::FUNDS,
};
...
struct create_metrics final : public core::metrics::Factory {
  explicit create_metrics(const std::string_view &group,
                          const std::string_view &function)
      : core::metrics::Factory(server::Flags::name(), group, function) {}
};
} // namespace

OrderEntry::OrderEntry(Handler &handler, core::io::Context &context,
                       uint16_t stream_id, Security &security, Shared &shared)
    : handler_(handler), stream_id_(stream_id),
      name_(roq::format("{}:{}:{}"_fmt, stream_id_, NAME,
                        security.get_account())),
      connection_(*this, context, Flags::decode_buffer_size(),
                  Flags::encode_buffer_size(), core::URI(Flags::rest_uri()),
                  ROQ_PACKAGE_NAME, KEEP_ALIVE, ALLOW_PIPELINING,
                  Flags::rest_request_timeout(),
                  Flags::rest_rate_limit_interval(),
                  Flags::rest_rate_limit_max_requests(),
                  Flags::rest_ping_freq(), Flags::rest_ping_path()),
      ...
      download_(Flags::rest_request_timeout(),
                [this](auto state) { return download(state); }) {}

bool OrderEntry::ready() const { return connection_.ready(); }

void OrderEntry::operator()(const Event<Start> &) { connection_.start(); }

void OrderEntry::operator()(const Event<Stop> &) { connection_.stop(); }
...
void OrderEntry::operator()(const Event<CreateOrder> &event,
...
void OrderEntry::operator()(const Event<ModifyOrder> &,
                            const std::string_view &,
                            const server::OMS_Order &order) {
  throw server::OMS_Exception(Error::MODIFY_ORDER_NOT_SUPPORTED, order);
}

void OrderEntry::operator()(const Event<CancelOrder> &event,
                            const std::string_view &request_id,
                            const server::OMS_Order &order) {
      ...
      [this, callback{std::move(callback)}](auto &response) {
        profile_.exchange_info([&]() {
          try {
            ...
            log::warn(R"(Exception type={}, what="{}")"_fmt, typeid(e).name(),
                      e.what());
...
void OrderEntry::get(
  ...
  auto headers =
      roq::format("X-MBX-APIKEY: {}\r\n"_fmt, security_.get_api_key());
  connection_.request(
      method, path, query, ACCEPT_ALL, {}, // content_type
      headers, {},                         // body
      {},                                  // QoS
      [this, callback{std::move(callback)}](auto &response) {
        profile_.account([&]() {
...
void OrderEntry::refresh_listen_key() {
  ...
  if (listen_key_refresh_ == listen_key_refresh_.zero() ||
      now < listen_key_refresh_)
    return;
...
void OrderEntry::create_order(
    const CreateOrder &create_order, const std::string_view &cl_ord_id,
  ...
  auto message = roq::format(
      ...
      R"("timestamp":{})"
      R"(}})"_fmt,
      create_order.symbol, side, type, time_in_force, create_order.quantity,
      0.0, create_order.price, cl_ord_id, 0.0, 0.0,
      Flags::rest_order_recv_window().count(), timestamp.count());
  log::debug(R"(body="{}")"_fmt, message);
  auto headers =
      roq::format("X-MBX-APIKEY: {}\r\n"_fmt, security_.get_api_key());
  connection_.request(
      method, path, {},                            // query
      ACCEPT_JSON, CONTENT_TYPE_JSON, headers, {}, // body
...
void OrderEntry::cancel_order(
    ...
    const std::string_view &request_id, const server::OMS_Order &order,
  ...
  auto message =
      roq::format(R"({{)"
                  ...
                  R"("timestamp":{})"
                  R"(}})"_fmt,
                  order.symbol, order.external_order_id, request_id,
                  Flags::rest_order_recv_window().count(), timestamp.count());
  log::debug(R"(body="{}")"_fmt, message);
  auto headers =
      roq::format("X-MBX-APIKEY: {}\r\n"_fmt, security_.get_api_key());
...
void OrderEntry::operator()(const json::ListenKey &listen_key) {
  ...
  if (utils::update(listen_key_, listen_key.listen_key)) {
    if (initial) {
      log::info(R"(Listen key has been acquired (value="{}"))"_fmt,
                listen_key_);
...
void OrderEntry::operator()(const json::ExchangeInfo &exchange_info) {
  ...
  for (const auto &item : exchange_info.symbols) {
    ...
    auto min_trade_vol =
        std::pow(10.0, -static_cast<double>(item.base_asset_precision));
  ...
  log::info("Exchange info: including symbols {}/{}"_fmt, counter,
