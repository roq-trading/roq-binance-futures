...
std::string_view WSAPIRequest::encode(std::string &buffer, WSAPIRequest const &request) {
  ...
  auto event_type = static_cast<uint8_t>(static_cast<WSAPIType::type_t>(request.type));
...
WSAPIRequest WSAPIRequest::decode(std::string_view const &buffer) {
  ...
  result.type = WSAPIType{static_cast<WSAPIType::type_t>(std::to_integer<uint8_t>(data[4]))};
