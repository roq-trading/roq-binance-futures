...
struct UserStreamParserTester final : public protocol::json::UserStreamParser::Handler {
 ...
 protected:
  ...
  void operator()(Trace<protocol::json::StrategyUpdate> const &event) override { dispatch_helper(event); }
  void operator()(Trace<protocol::json::GridUpdate> const &event) override { dispatch_helper(event); }
  void operator()(Trace<protocol::json::AccountConfigUpdate> const &event) override { dispatch_helper(event); }
  void operator()(Trace<protocol::json::TradeLite> const &event) override { dispatch_helper(event); }
  void operator()(Trace<protocol::json::ExecutionReport2> const &event) override { dispatch_helper(event); }
  void operator()(Trace<protocol::json::BalanceUpdate> const &event) override { dispatch_helper(event); }
  void operator()(Trace<protocol::json::LiabilityChange> const &event) override { dispatch_helper(event); }
  void operator()(Trace<protocol::json::OutboundAccountPosition> const &event) override { dispatch_helper(event); }
